...
void Adafruit_ILI9340::fillScreenPozzer(uint16_t color) {
...
//teste Pozzer
void Adafruit_ILI9340::printTextPozzer(int16_t x, int16_t y, int16_t w, int16_t h, unsigned char *str)
{
      int cont = 0;
      int bit  = 0;
      int byte = 0;
...
void Adafruit_ILI9340::printCompactTextGuilherme(int16_t x, int16_t y, int16_t w, int16_t h, unsigned char *str, uint8_t strSize){
   ...
   for(int idx = 0; idx < strSize; idx++)
   {
      ...
      for( int c = 0; c < str[idx]; c++)
      {
        ...
        //CLEAR_BIT(csport, cspinmask);
         ...
         if( flagBack == true)
         {
            //drawPixel(pX,pY,ILI9340_BLACK);
            hi = ILI9340_BLACK >> 8;
            lo = ILI9340_BLACK;
         }
         else
         {
            //drawPixel(pX,pY,ILI9340_WHITE);
            hi = ILI9340_WHITE >> 8;
            lo = ILI9340_WHITE;
         }
         col++;
