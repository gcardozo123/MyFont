...
Font::Font(const char * fname, unsigned int h)
{
	...
	this->familyName.append(face->family_name); //store the family name
	this->style.append(face->style_name);		//store font style	
	
	//Freetype measures font size in terms of 1/64ths of pixels. h<<6 = h*64
	FT_Set_Char_Size(face, h << 6, h << 6, 96, 96);
	
	//gera alfabeto da freetye (guarda em freetypeAlphabet)
	genFreetypeAlphabet();
	...
	writeAlphabetFileModeB(); //gera arquivo da fonte modo Alternado
	writeAlphabetFileModeA(); //gera arquivo da fonte modo Bit a Bit
...
void Font::genFreetypeAlphabet() {

	for (int ch = ASCII_ALPHABET_BEGIN; ch < ASCII_ALPHABET_BEGIN + NUM_CHARS; ch++){
		//Carrega glifo do nosso caractere
		if (FT_Load_Glyph(face, FT_Get_Char_Index(face, (char) ch), FT_LOAD_DEFAULT))
			throw std::runtime_error("FT_Load_Glyph failed");

		//Move a face do glifo para um objeto FT_Glyph
		FT_Glyph glyph;
		if (FT_Get_Glyph(face->glyph, &glyph))
			throw std::runtime_error("FT_Get_Glyph failed");

		//Converte o glifo para bitmap
		FT_Glyph_To_Bitmap(&glyph, ft_render_mode_normal, 0, 1);
		FT_BitmapGlyph bitmap_glyph = (FT_BitmapGlyph)glyph;

		//Referencia para facilitar o acesso ao bitmap
		FT_Bitmap& bitmap = bitmap_glyph->bitmap;
		
		//vetor para armazenamento do glifo atual: 
		//1 para pixel de foreground, 0 para background
		vector<int> currentCharacter; 
		
		/****
		Percorre o bitmap pixel a pixel armazenando 1 para 
		foreground e 0 para background. Ignora-se a informacao
		de antialiasing: se valor do pixel for maior que 100
		entao eh foreground, se nao eh background.
		****/
		for (int j = 0; j <bitmap.rows; j++) {
			for (int i = 0; i < bitmap.width; i++){
				unsigned char aux = bitmap.buffer[i + bitmap.width*j];
	...
	for (int c = 0; c < NUM_CHARS; c++){
		...
		for (int y = 0; y < height; y++)
		{
			for (int x = 0; x < width; x++)
			{
				...
				if (fontAlphabet[c][pos] == 1)
				{
					mask = 1 << (7 - bit);
					alphabetA[c][byte] |= mask;
				}
				cont++;
				bit = cont % 8;
				byte = cont / 8;
...
void Font::writeAlphabetFileModeB(){
	std::stringstream fontSize;
	fontSize << this->size;		//int to string
	string outputFileName = "../" + familyName + "_" + style + "_" + fontSize.str() + "_Alternado.txt";
	...
	{
		//File Header
		myFile << "Font: " << fontPath << " Size " << this->size << ". Character size "
			<< width << "x" << height << "px. Modo Alternado." << std::endl;
...
void Font::writeAlphabetFileModeA(){
	int length = ((width * height) / 8) + 1;
	
	std::stringstream fontSize;
	fontSize << this->size;		//int to string
	string outputFileName = "../" + familyName + "_" + style + "_" + fontSize.str() + "_Bit_a_Bit.txt";
	...
	if (myFile.is_open())
	{
		//File Header
		myFile << "Font: " << fontPath << " Size " << this->size << ". Character size "
			<< width << "x" << height << "px. Modo Bit a Bit." << std::endl;

		//Character dimensions
		myFile << width << " " << height << " pixels\n";

		//Character length
		myFile << length << " bytes per glyph\n";

		for (int c = 0; c < NUM_CHARS; c++){
			for (int i = 0; i < length; i++)
			{
				auto aux = alphabetA[c][i];
				myFile << (int)aux << " ";
			}
			myFile << std::endl;
		}
...
void Font::getDimensions(int *width, int *height){
	*width = this->width;
	*height = this->height;
}

void Font::getCharBit_a_Bit(char c, unsigned char arr[], int *length){
	int bit = 0, byte = 0, cont = 0;
	unsigned char mask = 0;
	*length = 0;
	int pixColor = 0;
...
void Font::getCharAlternado(char c, unsigned char arr[], int *length){
	int index = (int)c - ASCII_ALPHABET_BEGIN;
...
void Font::getCharGL(char c, unsigned char arr[], unsigned char r,
...
void Font::printFreetypeOnConsole(){
	for (int c = 0; c < NUM_CHARS; c++){
...
int Font::getMaxLengthB(){
	return findMax(charSizesAlph_B, NUM_CHARS);
}